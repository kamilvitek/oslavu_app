...
export function EventbriteTestComponent() {
  ...
  const runTest = async (endpoint: string, params: Record<string, string>) => {
    ...
    try {
      ...
      const url = `/api/analyze/events/eventbrite?${queryParams.toString()}`;
  ...
  const runAllTests = async () => {
    ...
    // Test 5: Search by category only
    await runTest("GET - Category Search", {
      city: testParams.city,
      category: testParams.category
    });
  };
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Configure the parameters for testing the Eventbrite API
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="pageSize">Results Size</Label>
              <Input
                id="pageSize"
                type="number"
                value={testParams.pageSize}
                onChange={(e) => setTestParams(prev => ({ ...prev, pageSize: e.target.value }))}
                placeholder="10"
              />
            </div>
          </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Run different API tests to verify the Eventbrite integration
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex flex-wrap gap-4">
            ...
            <Button 
              onClick={() => runTest("GET - Category Only", { city: testParams.city, category: testParams.category })}
              disabled={loading}
              variant="outline"
            >
              Test Category Only
      ...
      {results.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Results from the Eventbrite API tests
            </CardDescription>
          </CardHeader>
          <CardContent>
